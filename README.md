...
- **3 层上下文压缩管线**：大结果落盘 → 微压缩 → LLM 摘要，确保上下文不超限
...
| **Stop** | 一轮对话结束后 | 本轮 Token 消耗汇总 + 上下文占用统计（着色） |

### 上下文压缩

**每轮 AgentLoop 自动评估**、按 token 预算逐层触发的三层递进式压缩管线（所有阈值相对模型上下文窗口计算，窗口由 `.env` 中的模型名动态解析，陌生模型按 128K 兜底）：

1. **大结果落盘**：单条工具结果超上限（5% 窗口 token）时立即持久化至磁盘（sha256 内容寻址去重），上下文仅保留路径与头尾预览——大内容从源头不进上下文
2. **微压缩**：总 token 超过 50% 窗口时，旧工具结果**先落盘再替换**为占位符（无损，可随时重读）
3. **LLM 摘要**：总 token 超过 75% 窗口时调用 API **分段摘要**（单次输入恒有界，不会溢出），原始对话存档为 transcript，system 提示词与最近一轮完整工具周期保留；保留尾部再按 token 预算收口（同样无损，原文已落盘），避免单轮内堆积的文本撑爆窗口后压缩无法收敛

触发指标统一为 token——不按消息条数裁剪。条数阈值与窗口大小无关，在长窗口下会抢跑并饿死微压缩/摘要，且裁剪不落盘会不可逆地丢掉关键消息。

上下文溢出时另触发**应急反应式压缩**（压缩后重建请求，最多 3 次重试，可收敛）；每轮对话结束终端显示当前上下文占用（按 75%/95% 阈值着色）；`/compact` 命令可手动触发。压缩管线采用启发式 token 估算（CJK≈1、其他≈0.25/字符），不引入额外依赖。

**防重复读取四道防线**：占位符自带落盘指引（sha256/大小/分页提示）；已落盘文件清单注入 system 提示词（Brain 与子 Agent，最近 20 条环形淘汰）；同内容重复落盘自动告警；累计 ≥3 次触发熔断警告——LLM 不会陷入"读大文件→被压缩→再读"的循环。
...
│   │   ├── compaction_pipeline.py  # 3层上下文压缩管线（预算驱动/无损降级/分段摘要）
